@props([
    'label' => 'Select or type to select ...',
    'items' => [],
    'active' => null,
    'key' => 'id',
    'value' => 'name'
...
@php
    ...
    {
        $list[$items[$i]->$key] = $items[$i]->$value;
    }
@endphp
<!--
...
<div>
    ...
    <div class="relative mt-2">
        ...
        <ul class="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm" id="options" role="listbox">
            @foreach($list as $k => $v)
                @php
                    $classes['li'] = $active == $k ? "text-white bg-indigo-600" : "text-gray-900";
                    $classes['svg'] = $active == $k ? "text-white" : "text-indigo-600";
                @endphp
                <!--
                ...
                -->
                <li class="relative cursor-default select-none py-2 pl-8 pr-4 {{ $classes['li'] }}" id="option-0" role="option" tabindex="-1">
                    ...
                    -->
                    <span class="absolute inset-y-0 left-0 flex items-center pl-1.5 {{ $classes['li'] }}">
