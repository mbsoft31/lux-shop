...
use function Livewire\Volt\{form, mount, state, usesFileUploads};

$options = usesFileUploads();
$options->disk = 'public';
...
class InventoryItemForm extends Form
{
    ...
    {
        return 'BC-' . rand(10000000, 99999999);
    }

    public function generateSKU(): string
    {
        return 'SKU-' . rand(10000000, 99999999);
    }

    public function init($item, $inventory, $product): void
    {
        if ($item) {
            $this->sku = $item->sku;
    ...
    public function store(): void
    {
        ...
        $this->image->store(path: 'images/inventory', disk: 'public');
        $this->image = $this->image->hashName();

        $item = \App\Models\InventoryItem::make([
        ...
        session()->flash('status', 'inventory-saved');
    }
}
...
form(InventoryItemForm::class);
mount(function () {
    $this->form->init($this->item, $this->inventory, $this->product);
});
                            ...
                            <div class="mt-6 py-4 space-y-6 border-t">
                                ...
                                <div class="flex gap-6 flex-wrap flex-col sm:flex-row sm:items-center">
                                    <div class="h-14 w-14 flex-shrink-0">
                                        @if ($form->image && $form->image instanceof TemporaryUploadedFile)
